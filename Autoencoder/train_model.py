...
def train_model(train_with_clustering: bool, n_cluster_simulation: int, simulation_number: int,
    ...
    """ Trains the Autoencoder PyTorch model from autoencoder_py.py with the setting in config.py
    ...
    if train_with_clustering:
        model_path = f'models/{config.TRAIN_SIMULATION_TYPE}/n_cluster_{n_cluster_simulation}/' \
    ...
    train_data = train_data[:(len(train_data) - (len(train_data) % config.TRAIN_BATCH_SIZE))]
    ...
    train_data = torch.utils.data.DataLoader(train_data, batch_size=config.TRAIN_BATCH_SIZE, shuffle=True)

    validation_data = validation_data[:len(validation_data) - (len(validation_data) % config.TRAIN_BATCH_SIZE)]
    ...
    validation_data = torch.utils.data.DataLoader(validation_data, batch_size=config.TRAIN_BATCH_SIZE, shuffle=True)
...
if __name__ == '__main__':
    ...
    for n_cluster in config.TRAIN_N_CLUSTER_SIMULATION:
        for sim_number in config.TRAIN_SIMULATION_NUMBER:
